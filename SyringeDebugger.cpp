...
const std::unordered_map<int, std::string> ExitStatusMap
{
	{STATUS_GUARD_PAGE_VIOLATION,                "线程访问了被保护页的内存区域。"},
	{STATUS_LONGJUMP,                            "执行了 longjmp 操作。"},
	{STATUS_UNWIND_CONSOLIDATE,                  "异常展开已合并。"},
	{STATUS_INVALID_HANDLE,                      "使用了无效的句柄。"},
	{STATUS_INVALID_PARAMETER,                   "传递给函数的参数无效。"},
	{STATUS_NO_MEMORY,                           "内存不足，无法完成操作。"},
	{STATUS_DLL_NOT_FOUND,                       "找不到必需的动态链接库。"},
	{STATUS_ORDINAL_NOT_FOUND,                   "在 DLL 中找不到指定的导出序号。"},
	{STATUS_ENTRYPOINT_NOT_FOUND,                "找不到指定的入口点。"},
...
std::string GetExitStatusStr(int Status)
{
	auto it = ExitStatusMap.find(Status);
	if (it == ExitStatusMap.cend())
	{
		auto it2 = ExcMap.find(Status);
		if (it2 == ExcMap.cend())
		{
			return "";
		}
...
#pragma warning(pop)
	...
	else
	{
		...
		if (CheckBreakpoint)
		{
			...
			//--HOTFIX-- 0.3b3
			Log::WriteLine(__FUNCTION__ ": 让我康康程序发育正不正常啊~");
...
void SyringeDebugger::OutputProcModulePaths()
{
	...
	for (size_t i = 0; i < ModCount; i++)
	{
		...
		Log::WriteLine(__FUNCTION__ ": 模块(%d/%d): 路径%s", i + 1, ModCount + 1, ModPath);
	}
}
...
DWORD SyringeDebugger::HandleException(DEBUG_EVENT const& dbgEvent)
{
	...
	else if (exceptCode == DBG_CONTROL_C)
	{
		//Ctrl+C
		//From Console of Debuggee
		//Don't handle and let debuggee exit
		Log::WriteLine(__FUNCTION__ ": 收到来自调试目标的Ctrl+C中断，线程ID %u，地址 0x%08X",
			dbgEvent.dwThreadId,
			dbgEvent.u.Exception.ExceptionRecord.ExceptionAddress);
		Log::WriteLine(__FUNCTION__ ": 程序即将退出……");
		return DBG_EXCEPTION_NOT_HANDLED;
	}
	else if (exceptCode == DBG_CONTROL_BREAK)
	{
		//Ctrl+Break
		//From Console of Debuggee
		//Terminate the debuggee
		Log::WriteLine(__FUNCTION__ ": 收到来自调试目标的Ctrl+Break中断，线程ID %u，地址 0x%08X",
			dbgEvent.dwThreadId,
			dbgEvent.u.Exception.ExceptionRecord.ExceptionAddress);
		Log::WriteLine(__FUNCTION__ ": 已强制终止调试目标。");
